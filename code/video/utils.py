...
#================== Parsing variables from raw responses ======================

#==================== lines detected from OCR ===============================
...
def get_face_matches(response):
    
    '''
    Take a response object from Amazon rekognition GetFaceSearch and return a list of matched face IDs who were featured in video. 
    
    Use this function if only video-level face IDs are needed. 
    '''
    ...
    for personMatch in response['Persons']:
        if ('FaceMatches' in personMatch):
            for faceMatch in personMatch['FaceMatches']:
                FaceID = faceMatch['Face']['ExternalImageId']
...
def get_face_appearances(response):
    ...
    Use this function if timestamps of face appearances are needed. 
    '''
    ...
    for personMatch in response['Persons']:
        ...
        if ('FaceMatches' in personMatch):
            for faceMatch in personMatch['FaceMatches']:
                ...
                if FaceID in matches:
                    matches[FaceID].append(Timestamp)
                else:
                    matches[FaceID] = [Timestamp]              
